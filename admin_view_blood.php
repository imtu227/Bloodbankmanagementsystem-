...
$sql="Select hospital.HOSPITAL_NAME, blood.BLOOD_NAME,blood.BLOOD_ID
From state Inner Join
  country On blood.HOSPITAL_ID = hospital.HOSPITAL_ID ORDER BY BLOOD_ID desc $limit ";
								...
								$list.= "<table class='table table-striped' >";
									$list.= "<tr>
										...
										while($row=$result->fetch_assoc())
										{
											...
											$list.="<tr>";
												$list.= "<td>$i</td>";
												$list.= "<td>".$row["HOSPITAL_NAME"]."</td>";
												$list.= "<td>".$row["BLOOD_NAME"]."</td>";
												$list.= "<td><a href='admin_del_blood.php?id=".$row["BLOOD_ID"]."' class='btn btn-danger btn-sm'><i class='fa fa-trash'></i></a></td>";
											$list.="</tr>";
										}
								$list.= "</table>";
						}
